<?php
/**
 * mxLoggerRotate — ротация (автоудаление) старых записей лога mxLogger (MODX 3).
 *
 * Событие: OnMODXInit. Чтобы не работать на каждом запросе:
 *   - мгновенный выход, если ротация выключена (log_lifetime = 0);
 *   - троттлинг: реальная чистка не чаще раза в mxlogger.rotate_interval секунд
 *     (метка в кэше, партиция «mxlogger»);
 *   - удаление порциями (DELETE ... LIMIT) с лимитом порций за проход;
 *   - ошибки гасятся (OnMODXInit критичен).
 *
...
$cacheOpt = [xPDO::OPT_CACHE_KEY => 'mxlogger'];
$now = time();
$last = (int) $cm->get('rotate_last', $cacheOpt);
if ($last && ($now - $last) < $interval) {
    return; // ещё рано
}
// Метку ставим СРАЗУ, до удаления — чтобы параллельные запросы не дублировали чистку.
